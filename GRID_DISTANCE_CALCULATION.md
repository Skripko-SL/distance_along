# Расчёт расстояний от объектов до опорной сетки точек

Два метода расчёта:
1. **По прямой (great-circle)** — скрипт `calculate_distance.py`
2. **По дорожному графу** — скрипт `distance_along_roads.py`

---

## Исходные данные

### 1. Целевые объекты (school.dbf / .shp)

Файл в формате DBF (dBase/FoxPro), может сопровождаться shape-файлом.

| Поле | Тип | Описание |
|------|-----|----------|
| `id` | N | Уникальный идентификатор объекта |
| `X` | N | Долгота в градусах (WGS84, EPSG:4326) |
| `Y` | N | Широта в градусах (WGS84, EPSG:4326) |

Другие поля (название, адрес и т.п.) передаются в выходной CSV как есть.

Проекция исходных данных — **WGS84 (EPSG:4326)**, координаты в десятичных градусах.

### 2. Опорная сетка (points_buff_400.dbf / .shp)

Регулярная сетка точек с шагом **400 метров** в проекции Web Mercator (EPSG:3857).

| Поле | Тип | Описание |
|------|-----|----------|
| `id` | N | Уникальный идентификатор точки сетки |
| `left` | N | X-координата левого края ячейки (EPSG:3857, метры) |
| `top` | N | Y-координата верхнего края ячейки (EPSG:3857, метры) |
| `right` | N | X-координата правого края ячейки (EPSG:3857, метры) |
| `bottom` | N | Y-координата нижнего края ячейки (EPSG:3857, метры) |
| `col_index` | N | Индекс колонки в сетке |
| `row_index` | N | Индекс строки в сетке |

Центр ячейки: `center_x = left + 200`, `center_y = top + 200`.

Проекция исходных данных — **WGS 84 Web Mercator Auxiliary Sphere (EPSG:3857)**.

---

## Метод расчёта

### Этап 1. Приведение координат к единой системе отсчёта

Координаты опорных точек сетки пересчитываются из **EPSG:3857 (метры)** в **WGS84 (градусы)**:

```
lon = cx / 20037508.34 * 180
lat = degrees(2 * atan(exp(cy / 20037508.34 * π)) - π/2)
```

где:
- `cx, cy` — центр ячейки сетки в EPSG:3857
- `20037508.34 = π * R` (R = 6378137 м — радиус сферы WGS84)

### Этап 2. Построение пространственного индекса (KD-дерево)

Для эффективного поиска ближайшего соседа среди 114 232 опорных точек используется **KD-дерево** (scipy.spatial.cKDTree).

**Проблема:** Евклидово расстояние на плоскости (широта, долгота) в радианах искажено — на широте 56° 1° долготы в ~1.8 раза короче 1° широты. Простой поиск по (lat, lon) даёт неверный результат.

**Решение:** Точки проецируются на **единичную сферу (3D-декартовы координаты)**:

```
x = cos(lat_rad) * cos(lon_rad)
y = cos(lat_rad) * sin(lon_rad)
z = sin(lat_rad)
```

Евклидово расстояние в 3D-пространстве монотонно связано с гаверсинусовым расстоянием на сфере, поэтому поиск ближайшего соседа в 3D-координатах **корректен для любых расстояний**.

### Этап 3. Поиск ближайшей точки

Для каждого целевого объекта:
1. Его координаты (lat, lon) переводятся в 3D-декартовы координаты на единичной сфере
2. Через KD-дерево находится ближайшая опорная точка (k=1)

### Этап 4. Расчёт геодезического расстояния

Финальное расстояние вычисляется по **формуле гаверсинуса**:

```
a = sin²(Δlat/2) + cos(lat₁) · cos(lat₂) · sin²(Δlon/2)
c = 2 · atan2(√a, √(1-a))
distance = c · R
```

где R = 6 371 000 м — средний радиус Земли.

Это даёт точное расстояние по **большому кругу** (great-circle distance).

---

## Результаты расчёта (школы → сетка)

### Параметры запуска

| Параметр | Значение |
|----------|----------|
| Целевые объекты | 821 школа Нижегородской области |
| Опорная сетка | 114 232 точки, шаг 400 м |
| Время выполнения | ~0.5 секунды |
| Выходной файл | `school_to_grid_distance.csv` |

### Статистика расстояний

| Показатель | Значение |
|------------|----------|
| Минимальное | 25.2 м |
| 25-й перцентиль | 318.2 м |
| Медиана (50%) | 590.9 м |
| 75-й перцентиль | 1 917.7 м |
| 90-й перцентиль | 5 208.8 м |
| 95-й перцентиль | 6 624.7 м |
| 99-й перцентиль | 7 915.9 м |
| Максимальное | 8 917.1 м |

### Структура выходного CSV

```
school_id, school_id_t, school_name, school_address,
school_x, school_y,
grid_point_id, grid_left, grid_top, grid_col, grid_row,
distance_m
```

| Поле | Описание |
|------|----------|
| `school_id` | ID школы из school.dbf |
| `school_id_t` | Уникальный идентификатор школы |
| `school_name` | Название школы |
| `school_address` | Адрес школы |
| `school_x` | Долгота школы (WGS84, градусы) |
| `school_y` | Широта школы (WGS84, градусы) |
| `grid_point_id` | ID ближайшей опорной точки сетки |
| `grid_left` | Координата left точки (EPSG:3857, метры) |
| `grid_top` | Координата top точки (EPSG:3857, метры) |
| `grid_col` | Индекс колонки точки в сетке |
| `grid_row` | Индекс строки точки в сетке |
| `distance_m` | Расстояние от школы до точки (метры, great-circle) |

### Примечание о покрытии сетки

Сетка `points_buff_400` не является полным прямоугольником — она покрывает **114 232 точки** (при потенциальном максимуме ~2.8 млн для полного прямоугольника над областью). Некоторые территории (включая часть Нижнего Новгорода) находятся за пределами сетки. Для 37.5% школ расстояние до ближайшей точки превышает 1 км — это школы, попавшие в пропуски сетки.

---

## Расчёт расстояний по дорожному графу (distance_along_roads.py)

Этот подход находит кратчайший путь от каждой точки сетки до ближайшей школы
по дорожной сети. Полное расстояние складывается из трёх компонент:

```
total_distance = перпендикуляр(школа → дорога)
               + расстояние_по_графу(проекция_школы → проекция_точки)
               + перпендикуляр(точка_сетки → дорога)
```

### Исходные данные

Дополнительно к school.dbf и points_buff_400.dbf требуется:

| Файл | Описание |
|------|----------|
| `roads.shp` / `.dbf` / `.shx` | Дорожная сеть Нижегородской области (OSM, EPSG:3857), 219 187 сегментов |

Поля дорожной сети:

| Поле | Описание |
|------|----------|
| `fclass` | Класс дороги (primary, secondary, tertiary, residential, service, track, footway, path, etc.) |
| `oneway` | Направление: 'B' — двусторонняя, 'F' — односторонняя вперёд, 'T' — односторонняя назад |
| `leght` | Длина сегмента в метрах (предвычислена) |
| `name` | Название улицы |

### Метод

#### Этап 1. Построение дорожного графа и индекса сегментов

1. Из каждого сегмента дороги (LineString) извлекаются начальная и конечная точки, которые становятся узлами графа. Вес ребра = длина сегмента (`leght`).
2. Односторонние дороги (`oneway='F'` / `'T'`) учитываются как направленные рёбра.
3. Строится пространственный индекс (KD-дерево) из **всех вершин полилиний** (1 950 725 точек), а не только концов. Каждая вершина хранит ссылку на свой сегмент.

Характеристики графа:
- Узлов: ~330 000
- Рёбер: ~433 000
- Всего вершин полилиний: ~1 950 000
- Компонент связности: ~115 000

#### Этап 2. Проекция школ на дорожный граф (перпендикуляр)

Для каждой школы:
1. Через KD-дерево находятся K=3 ближайших вершины дорог
2. Для каждой уникальной дороги-кандидата вычисляется перпендикуляр от школы до полилинии
3. Выбирается дорога с **минимальным перпендикуляром** → `perp_school`
4. Определяется **доля** (`frac_s`) — положение основания перпендикуляра вдоль сегмента (0.0 = начало, 1.0 = конец)
5. Расстояние от школы до концов сегмента:
   - `dist_to_n1 = frac_s * L` (до начала)
   - `dist_to_n2 = (1 - frac_s) * L` (до конца)
6. Для каждого концевого узла сегмента запоминается минимальное расстояние до школы:
   - `school_dist[node] = min(school_dist[node], perp_school + dist_along_segment)`

#### Этап 3. Супер-источник и мульти-источниковый Dijkstra

Создаётся **виртуальный супер-узел**, соединённый рёбрами со всеми узлами, к которым есть доступ от школ. Вес ребра = `school_dist[node]`.

```
супер-узел ──[school_dist]──→ концевой_узел_A
          ──[school_dist]──→ концевой_узел_B
          ...
```

Запускается алгоритм Дейкстры от супер-узла. Результат — для каждого узла графа минимальное расстояние до ближайшей школы (уже включая перпендикуляр школы и смещение вдоль сегмента).

#### Этап 4. Проекция точек сетки на дорожный граф (перпендикуляр)

Для каждой точки сетки выполняется та же процедура, что и для школ (шаг 2):
1. Поиск ближайшей дороги (KD-дерево, K=3 кандидата)
2. Вычисление перпендикуляра → `perp_grid`
3. Определение доли вдоль сегмента → `frac_g`
4. Расстояние от точки до концов сегмента: `d_n1 = frac_g * L`, `d_n2 = (1 - frac_g) * L`

#### Этап 5. Формирование итогового расстояния

Для каждой точки сетки:

```
road_dist = min(
    d_n1 + node_dist[n1],
    d_n2 + node_dist[n2]
)
total = perp_grid + road_dist
```

где `node_dist[n]` — уже включает `perp_school` + смещение школы вдоль её сегмента.

### Результаты расчёта

| Параметр | Значение |
|----------|----------|
| Целевые объекты | 821 школа |
| Опорная сетка | 114 232 точки |
| Дорожный граф | 330 270 узлов, 433 311 рёбер |
| Индекс полилиний | 1 950 725 точек |
| Выходной файл | `grid_to_school_distance.csv` |

#### Статистика

| Показатель | Значение |
|------------|----------|
| Всего точек сетки | 114 232 |
| Достижимо по дорогам | **25 365 (22.2%)** |
| Перпендикуляр (медиана) | 275 м |

**Road distance (достижимые точки, граф + смещение по сегменту):**

| Показатель | Значение |
|------------|----------|
| Минимальное | 58 м |
| 25-й перцентиль | 7 054 м |
| Медиана (50%) | 15 317 м |
| 75-й перцентиль | 28 799 м |
| 90-й перцентиль | 47 967 м |
| 95-й перцентиль | 71 110 м |
| 99-й перцентиль | 107 806 м |
| Максимальное | 220 590 м |

**Total distance (перпендикуляр + road):**

| Показатель | Значение |
|------------|----------|
| Минимальное | 206 м |
| Медиана | 15 653 м |
| Максимальное | 220 915 м |

#### Структура выходного CSV

```
grid_point_id, grid_lon, grid_lat, grid_col, grid_row,
perp_dist_m, road_dist_to_school_m, total_dist_m
```

| Поле | Описание |
|------|----------|
| `grid_point_id` | ID точки сетки |
| `grid_lon` | Долгота точки (WGS84) |
| `grid_lat` | Широта точки (WGS84) |
| `grid_col` / `grid_row` | Индексы колонки и строки в сетке |
| `perp_dist_m` | Перпендикуляр от точки сетки до ближайшей дороги (м) |
| `road_dist_to_school_m` | Расстояние по дорогам (граф + смещение по сегменту) (м), или `NaN` |
| `total_dist_m` | Полное расстояние: перп + road (м), или `NaN` |

#### Важное примечание о связности графа

Дорожная сеть из shape-файла **не является топологически связной**. Из 219 187 сегментов только ~22% соединены в компоненты, достижимые из школ. Это связано с тем, что:

- OSM → shapefile теряет информацию об общих узлах (node IDs) между дорогами
- Каждый сегмент (way) хранится как независимая полилиния
- Даже соседние сегменты, которые в реальности пересекаются, могут иметь концы, различающиеся на сантиметры

Для 77.8% точек сетки дорожное расстояние недоступно — они находятся в изолированных компонентах дорожного графа (отдалённые деревни, лесные дороги, пешеходные тропы без связи с основной сетью, содержащей школы).

---

## Использование для других объектов

### Подготовка данных

Для расчёта расстояний от **любых других объектов** (больницы, остановки, магазины, etc.) необходимо подготовить CSV-файл со следующими колонками:

```csv
id,lat,lon,name,address
1,56.3265,43.8794,"Объект 1","Адрес 1"
2,56.2175,43.7618,"Объект 2","Адрес 2"
```

Минимально обязательные поля: `id`, `lat`, `lon`.

### Запуск расчёта

```bash
python3 calculate_distance.py \
  --objects-file objects.csv \
  --objects-crs epsg:4326 \
  --grid-file points_buff_400.dbf \
  --output results.csv
```

### Пример: больницы

```python
from dbfread import DBF
import numpy as np
from scipy.spatial import cKDTree
import math, csv

# 1. Загрузка точек сетки
def load_grid(grid_path):
    # ... (см. calculate_distance.py: load_grid)
    pass

# 2. Загрузка больниц
hospitals = []
with open('hospitals.csv') as f:
    for row in csv.DictReader(f):
        hospitals.append({
            'id': int(row['id']),
            'lat': float(row['lat']),
            'lon': float(row['lon']),
            'name': row.get('name', ''),
        })

# 3. Построение KD-дерева
points = load_grid('points_buff_400.dbf')
p_lats = np.radians([p['lat'] for p in points])
p_lons = np.radians([p['lon'] for p in points])
tree = cKDTree(np.column_stack([
    np.cos(p_lats) * np.cos(p_lons),
    np.cos(p_lats) * np.sin(p_lons),
    np.sin(p_lats),
]))

# 4. Поиск ближайших
h_lats = np.radians([h['lat'] for h in hospitals])
h_lons = np.radians([h['lon'] for h in hospitals])
h_3d = np.column_stack([
    np.cos(h_lats) * np.cos(h_lons),
    np.cos(h_lats) * np.sin(h_lons),
    np.sin(h_lats),
])
_, indices = tree.query(h_3d, k=1)

# 5. Расчёт расстояний и сохранение
EARTH_R = 6371000
with open('hospitals_to_grid.csv', 'w', newline='', encoding='utf-8') as f:
    w = csv.writer(f)
    w.writerow(['hospital_id', 'lat', 'lon', 'grid_point_id', 'grid_col', 'grid_row', 'distance_m'])
    for i, h in enumerate(hospitals):
        p = points[indices[i]]
        # haversine ...
        w.writerow([h['id'], h['lat'], h['lon'], p['id'], p['col_index'], p['row_index'], dist_m])
```

### Форматы входных данных

Скрипт поддерживает любые форматы, читаемые Python:

| Формат | Библиотека | Пример |
|--------|-----------|--------|
| DBF (dBase/FoxPro) | `dbfread` | `DBF(path, raw=True)` |
| CSV | встроенный `csv` | `csv.DictReader(open(path))` |
| Shapefile | `geopandas` / `fiona` | `gpd.read_file(path)` |
| GeoJSON | `geopandas` / `json` | `gpd.read_file(path)` |
| Excel | `pandas` / `openpyxl` | `pd.read_excel(path)` |
| PostGIS | `sqlalchemy` | `pd.read_sql(query, conn)` |

### Требования к системе координат

| Данные | Ожидаемая проекция |
|--------|--------------------|
| Целевые объекты | **WGS84 (EPSG:4326)**, координаты `(lat, lon)` в градусах |
| Опорная сетка | **EPSG:3857** (Web Mercator), координаты в метрах |

Если ваши данные в другой проекции — перед расчётом необходимо выполнить перепроецирование (например, через `pyproj.Transformer`).

### Зависимости

#### Для прямолинейного расчёта
```bash
pip3 install scipy numpy dbfread
```

#### Для расчёта по дорожному графу (с перпендикулярами)
```bash
pip3 install scipy numpy dbfread pyshp
```

Установлено в системе.

---

## Файлы проекта

| Файл | Описание |
|------|----------|
| `calculate_distance.py` | Основной скрипт расчёта (шаблон для адаптации) |
| `school.dbf` | Исходные данные — 821 школа |
| `school.shp` / `.shx` / `.prj` / `.cpg` | Shape-файл школ |
| `points_buff_400.dbf` | Опорная сетка — 114 232 точки |
| `points_buff_400.shp` / `.shx` / `.prj` / `.cpg` | Shape-файл сетки |
| `school_to_grid_distance.csv` | Результат расчёта (по прямой) |
| `grid_to_school_distance.csv` | Результат расчёта (по дорожному графу) |
| `distance_along_roads.py` | Скрипт расчёта по дорожному графу |
| `school_analysis.md` | Анализ данных школ |
| `GRID_DISTANCE_CALCULATION.md` | Данный файл — описание методологии |
