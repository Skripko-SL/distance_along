...
Скрипт `calculate_distance.py` находит для каждого заданного объекта (например, школы) **ближайшую опорную точку** из регулярной сетки `points_buff_400` и рассчитывает **геодезическое расстояние** между ними.

Результат сохраняется в CSV-файл для последующего анализа и визуализации.
...
| `school_to_grid_distance.csv` | Результат расчёта |
